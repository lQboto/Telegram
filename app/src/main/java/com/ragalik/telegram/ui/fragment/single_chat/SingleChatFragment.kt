...
import com.ragalik.telegram.ui.fragment.message_recycler_view.view.AppViewFactory
    ...
    private fun initFields() {
        ...
        chat_input_message.addTextChangedListener(AppTextWatcher {
            ...
            if (string.isEmpty() || string == "Запись") {
                chat_bth_send_message.visibility = View.GONE
                chat_bth_attach.visibility = View.VISIBLE
                chat_bth_voice.visibility = View.VISIBLE
            } else {
                chat_bth_send_message.visibility = View.VISIBLE
                chat_bth_attach.visibility = View.GONE
                chat_bth_voice.visibility = View.GONE
            }
        })

        chat_bth_attach.setOnClickListener {
            attachFile()
        }

        CoroutineScope(Dispatchers.Main).launch {
            chat_bth_voice.setOnTouchListener { view, motionEvent ->
                if (checkPermission(RECORD_AUDIO)) {
                    if (motionEvent.action == MotionEvent.ACTION_DOWN) {
                        chat_input_message.setText("Запись")
                        chat_bth_voice.setColorFilter(ContextCompat.getColor(APP_ACTIVITY, R.color.primary))
                        val messageKey = getMessageKey(contact.id)
                        mAppVoiceRecorder.startRecord(messageKey)
                    } else if (motionEvent.action == MotionEvent.ACTION_UP) {
                        chat_input_message.setText("")
                        chat_bth_voice.colorFilter = null
